import os, json, asyncio, re, string, random, aiohttp, time, glob
...
class UltraUsernameClaimer:
    ...
    def find_session_files(self):
        """البحث عن ملفات الجلسات في مجلد sessions"""
        sessions_dir = "sessions"
        if not os.path.exists(sessions_dir):
            os.makedirs(sessions_dir)
            return []
        
        # البحث عن جميع ملفات الجلسات
        session_files = []
        for file in os.listdir(sessions_dir):
            if file.endswith('.session'):
                session_files.append(os.path.join(sessions_dir, file))
    ...
    async def auto_login_with_session(self):
        ...
        try:
            print(f"{Colors.CYAN}🔐 البحث عن جلسات مسجلة...{Colors.RESET}")
            
            # البحث عن ملفات الجلسات
            self.session_files = self.find_session_files()
            
            if not self.session_files:
                print(f"{Colors.YELLOW}⚠️  لم يتم العثور على أي جلسات مسجلة{Colors.RESET}")
                return False
            
            # إذا كان هناك جلسة واحدة، استخدمها تلقائياً
            if len(self.session_files) == 1:
                self.selected_session = self.session_files[0]
                print(f"{Colors.GREEN}📁 تم العثور على جلسة: {os.path.basename(self.selected_session)}{Colors.RESET}")
            else:
                # إذا كان هناك أكثر من جلسة، عرض قائمة للاختيار
                print(f"{Colors.CYAN}📋 تم العثور على {len(self.session_files)} جلسات:{Colors.RESET}")
                for i, session_file in enumerate(self.session_files, 1):
                    print(f"{i}. {os.path.basename(session_file)}")
                
                try:
                    choice = int(input(f"{Colors.YELLOW}⌨️  اختر رقم الجلسة (1-{len(self.session_files)}): {Colors.RESET}"))
                    if 1 <= choice <= len(self.session_files):
                        self.selected_session = self.session_files[choice - 1]
                    else:
                        print(f"{Colors.RED}❌ اختيار غير صحيح{Colors.RESET}")
                        return False
                except ValueError:
                    print(f"{Colors.RED}❌ يجب إدخال رقم صحيح{Colors.RESET}")
                    return False
            
            # تحميل الجلسة المحددة
            print(f"{Colors.CYAN}🔐 محاولة تسجيل الدخول باستخدام {os.path.basename(self.selected_session)}...{Colors.RESET}")
            
            # استخراج رقم الهاتف من اسم الملف
            session_name = os.path.basename(self.selected_session).replace('.session', '')
            
            # إنشاء العميل باستخدام الجلسة
            self.client = TelegramClient(self.selected_session, api_id, api_hash)
